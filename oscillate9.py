...
def makeSignal(t, phase, w=2*np.pi, mag=0.95, dt=1e-3):
    idx = int(phase * t / dt)
...
def go(neuron_type, t=10, seed=0, dt=0.001, nPre=300, nEns=100, w=2*np.pi,
    m=Uniform(40, 80), i=Uniform(-0.5, 0.5), stim_func1=lambda t: 0, stim_func2=lambda t: 0, tSupv=0.1,
    ...
    with nengo.Network() as model:
        ...
        nengo.Connection(tarA, tarAX, synapse=fTarget)
        ...
        if test:
            ...
            nengo.Connection(tarA, tarA, synapse=fTarget, function=feedback)
...
def run(neuron_type, nTrain, nTest, tTrain, tTest, eRate, tTransTrain=0, tTrans=0, nTrainDF=10,
    ...
    if 0 in load:
        ...
        for n in range(nTrain):
            ...
            d0, e0, w0 = data['d0'], data['e0'], data['w0']
            np.savez(f"data/oscillate9_{neuron_type}.npz", d0=d0, e0=e0, w0=w0)
    ...
    if 1 in load:
        ...
        d1, tauRise1, tauFall1 = trainDF(spikes, targets, nTrainDF,
            dt=dt, network="oscillate9", neuron_type=neuron_type, ens="ens", penalty=penalty, reg=1e-1)
        d2, tauRise2, tauFall2 = trainDF(spikes, targets2, nTrainDF,
            dt=dt, network="oscillate9", neuron_type=neuron_type, ens="ens", penalty=penalty)
        f1 = DoubleExp(tauRise1, tauFall1)
        f2 = DoubleExp(tauRise2, tauFall2)
        np.savez(f"data/oscillate9_{neuron_type}.npz",
            d0=d0, e0=e0, w0=w0,
        ...
        fig.savefig(f'plots/oscillate9/decode_{neuron_type}.pdf')
    ...
    if 2 in load:
        ...
        fig.savefig(f'plots/oscillate9/check_{neuron_type}.pdf')
    ...
    for n in range(nTest):
        ...
        fig.savefig(f'plots/oscillate9/test_{neuron_type}.pdf')
...
def compare(neuron_types, nTrain=10, nTrainDF=10, tTrain=10, nTest=1, tTest=5, tTrans=5, load=[],
    eRates=[1e-7, 3e-7, 1e-7, 3e-8], tTransTrains=[0, 5, 5, 10]):
...
compare([LIF()], nTrain=15, nTrainDF=10, tTrain=10, eRates=[3e-7], load=[])