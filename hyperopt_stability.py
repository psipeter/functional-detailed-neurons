...
def run(neuron_type, nTrain, nTest, tTrain, tTest, eRate,
    ...
    if 1 in load:
        data = np.load(f"data/hyperopt_stability_{neuron_type}.npz")
        ...
        for n in range(nTrain):
            ...
            d, e, w = data['d'], data['e'], data['w']
            np.savez(f"data/hyperopt_stability_{neuron_type}.npz", d=d, e=e, w=w)
    ...
    if 2 in load:
        ...
        for i in range(nTest):
            d, rise, fall = trainDF(spikes, targets, nTrain, seed=i, evals=100,
                dt=dt, network="hyperopt_stability", ens='ens', neuron_type=neuron_type)
            # print(rise, fall)
            rises.append(rise)
            falls.append(fall)
        rises = np.array(rises)
        falls = np.array(falls)
        np.savez(f"data/hyperopt_stability_{neuron_type}.npz", d=d, e=e, w=w, rises=rises, falls=falls)
